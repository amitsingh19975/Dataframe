...
template <Series ValueType> struct basic_frame_view<ValueType> {
    ...
    struct iterator {
        ...
        constexpr iterator(series_pointer ptr, slice_type cs, slice_type rs) noexcept
    ...
    constexpr size_type rows() const noexcept { return m_rows ? m_row_slice.size() : 0u; }
    constexpr size_type cols() const noexcept { return m_cols ? m_col_slice.size() : 0u; }
    ...
    constexpr basic_frame_view operator()(slice_type cs, slice_type rs = {}) const noexcept{
        return {m_data, m_col_slice(cs), m_row_slice(rs), m_cols, m_rows};
    }

    constexpr basic_frame_view<ValueType const> operator()(slice_type cs, slice_type rs = {}) noexcept{
        return {m_data, m_col_slice(cs), m_row_slice(rs), m_cols, m_rows};
    }
