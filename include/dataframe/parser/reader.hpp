...
struct reader {
    ...
    reader(std::string_view filename, std::string_view sep = "\n")
        : m_file(filename.data(), std::ios::binary), m_sep(std::move(sep)) {
        if (!m_file) {
    ...
    inline iterator begin() noexcept { return iterator{m_file}; }
    ...
    inline std::optional<std::string> try_next() {
        ...
        while ((m_curr_sep = next())) {
            if (is_sep(static_cast<char>(m_curr_sep)) || m_curr_sep == EOF)
                break;
    ...
    inline int_type next() { return m_file.get(); }
    ...
    inline void seek(pos_type pos) { m_file.seekg(pos); }

    inline void skip() { (void)next(); }

    inline void seek(off_type off, std::ios_base::seekdir way) {
        m_file.seekg(off, way);
    }

    inline pos_type tell() { return m_file.tellg(); }

    inline int_type peek() { return m_file.peek(); }
    ...
    inline bool has_next() { return !m_file.fail(); }
    ...
    inline std::size_t lines() {
        auto cp = tell();

        int_type new_line = static_cast<int_type>('\n');
        int_type c;
        std::size_t l{};
        while ((c = next()) != EOF) {
            if (c == new_line)
    ...
    constexpr bool parsed() const noexcept { return m_parsed; }

    constexpr void parsed(bool f) noexcept { m_parsed = f; }
