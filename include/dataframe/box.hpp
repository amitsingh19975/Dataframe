...
template <typename... Ts> struct basic_box {
    ...
    basic_box(std::string u, DType auto dtype)
        : m_data(std::move(u)), m_index(get<basic_box>(dtype)) {
        if (!is_valid()) {
            throw std::runtime_error(
    ...
    constexpr bool is_valid() const noexcept {
        return (m_index == 0u) || empty() || (m_data.index() == m_index);
    }

  private:
    template <typename T> constexpr void construct(T &&val) {
        if (m_index == 0u) {
            m_data.template emplace<T>(std::move(val));
        } else {
            tuple_for<type_list>([&val, this]<typename I>(I) {
                ...
                if constexpr (std::is_same_v<t_type, element_type>) {
                    if (m_index == idx) {
                        m_data.template emplace<element_type>(std::move(val));
                        return;
                    }
                }
