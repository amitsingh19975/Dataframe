...
template <typename Fn, typename In>
using invoke_fn_result_t =
    typename invoke_fn_result<std::decay_t<Fn>, In>::type;
...
requires(is_tuple_v<Input> &&is_tuple_v<TupleType>) constexpr decltype(auto)
    ...
    if constexpr (sz > I + 1u) {
        ...
        return invoke_pipeline_left_to_right<I + 1>(
            ...
            std::forward_as_tuple(std::forward<decltype(next_in)>(next_in)));
...
requires(is_tuple_v<TupleType> &&is_tuple_v<Input>) constexpr decltype(auto)
    invoke_pipeline(TupleType &&t, Input &&in) noexcept {
...
template <typename TupleType, typename Input>
requires(is_tuple_v<TupleType> &&is_tuple_v<Input>) constexpr decltype(auto)
    invoke_pipeline(TupleType &&t, Input &&in, tag::right_to_left_t) noexcept {
    return invoke_pipeline_right_to_left(std::forward<TupleType>(t),
                                         std::forward<Input>(in));
}

template <typename... Ts> constexpr decltype(auto) make_args(Ts &&... args) {
