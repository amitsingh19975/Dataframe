...
namespace amt {
    ...
    class basic_unbounded_storage {
        ...
        static_assert( TypesUpperBound != 0,
                       "Storage cannot hold any type, please set upper limit "
                       "greater than 0 or set to NoUpperLimit" );
        using storage_trait_type = S;

        struct function_table {
            using size_fn_t =
                std::function< std::size_t( basic_unbounded_storage const& ) >;
            using copy_fn_t = std::function< void(
                basic_unbounded_storage&, basic_unbounded_storage const& ) >;
            using move_fn_t = std::function< void( basic_unbounded_storage&,
                                                   basic_unbounded_storage& ) >;
            using destroy_fn_t =
                std::function< void( basic_unbounded_storage& ) >;
            using equal_fn_t =
                std::function< bool( basic_unbounded_storage const&,
                                     basic_unbounded_storage const& ) >;

            constexpr function_table() noexcept = default;

            function_table( size_fn_t sz, copy_fn_t cp, move_fn_t mv,
                            destroy_fn_t des, equal_fn_t bin,
                            std::type_index index ) noexcept
                : m_size_fn( std::move( sz ) ), m_copy_fn( std::move( cp ) ),
                  m_move_fn( std::move( mv ) ),
                  m_destroy_fn( std::move( des ) ),
                  m_equal_fn( std::move( bin ) ), m_type_index( index ) {}
            ...
            static constexpr function_table make() noexcept {
                return function_table(
                    ...
                    []( basic_unbounded_storage& self,
                        basic_unbounded_storage& other ) {
                        return m_data< U >[ &self ] =
                                   std::move( m_data< U >[ &other ] );
                    },
                    []( basic_unbounded_storage& self ) {
                    ...
                    []( basic_unbounded_storage const& self,
                        ...
                        if ( !lopt || !ropt )
                            return false;
                        if ( ( *lopt ).size() != ( *ropt ).size() )
                            return false;
                        return std::equal( ( *lopt ).begin(), ( *lopt ).end(),
                                           ( *ropt ).begin() );
                    },
                    { typeid( U ) } );
            }
            ...
            constexpr auto
            copy( basic_unbounded_storage& self,
                  basic_unbounded_storage const& other ) const noexcept
                -> void {
                std::invoke( m_copy_fn, self, other );
            }

            constexpr auto move( basic_unbounded_storage& self,
                                 basic_unbounded_storage& other ) const noexcept
                -> void {
                std::invoke( m_move_fn, self, other );
            }

            constexpr auto
            equal( basic_unbounded_storage const& self,
                   basic_unbounded_storage const& other ) const noexcept
                -> bool {
            ...
            constexpr auto
            size( basic_unbounded_storage const& self ) const noexcept
                -> std::size_t {
                return std::invoke( m_size_fn, self );
            }

            constexpr auto
            destroy( basic_unbounded_storage& self ) const noexcept -> void {
    ...
    public:
        ...
        template< typename T >
        using base_type = std::unordered_map< basic_unbounded_storage const*,
        ...
        requires is_static_storage constexpr basic_unbounded_storage(
            std::initializer_list< U > const& li ) {
            auto& temp = get_container< U >();
            if ( temp.size() < li.size() ) {
                throw std::runtime_error( "amt::basic_unbounded_storage(std::"
                                          "initializer_list<U> const&) "
                                          ": "
                                          "the number of elements inside the "
                                          "initializer list exceeds "
                                          "the static container size" );
            }
        ...
        template< typename T >
        constexpr auto data() -> optinal_storage_type< T >& {
            return get_container< T >();
        }

        template< typename T >
        constexpr auto data() const -> optinal_storage_type< T > const& {
            return get_container< T >();
        }

        constexpr auto size() const noexcept -> size_type {
            auto const sz =
                ( has_upper_limit ? m_curr_pos : m_fn_table.size() );
            size_type s {};
        ...
        constexpr auto
        operator==( basic_unbounded_storage const& other ) const noexcept
            -> bool {
            auto const sz =
                ( has_upper_limit ? m_curr_pos : m_fn_table.size() );
            for ( auto i = 0ul; i < sz; ++i ) {
                if ( !m_fn_table[ i ].equal( *this, other ) )
                    return false;
            }
            return true;
        }
        ...
        constexpr auto get_or_make( Ts&&... ts ) noexcept
            -> optinal_storage_type< T >& {
            auto& opt = get_container< T >();
            if ( !opt )
                opt = std::move(
                    storage_type< T >( std::forward< Ts >( ts )... ) );
            return opt;
        }

    private:
        constexpr auto
        copy_helper( basic_unbounded_storage const& other ) noexcept -> void {
            auto const sz =
                ( has_upper_limit ? m_curr_pos : m_fn_table.size() );
        ...
        constexpr auto move_helper( basic_unbounded_storage& other ) noexcept
            -> void {
            auto const sz =
                ( has_upper_limit ? m_curr_pos : m_fn_table.size() );
        ...
        constexpr auto get_container() const
            ...
            if ( auto it = item.find( this ); it == item.end() ) {
                throw std::runtime_error(
                    ...
                    "container with the type T does not exist, and const "
                    "function "
                    "cannot create the container." );
            } else {
                return it->second;
            }
        }
    ...
    template< typename T >
    typename basic_unbounded_storage< S, N >::template base_type< T >
        basic_unbounded_storage< S, N >::m_data;

    using dynamic_unbounded_storage =
        basic_unbounded_storage< dynamic_storage<> >;

    template< std::size_t N >
    using static_unbounded_storage =
        basic_unbounded_storage< static_storage< N > >;
