...
namespace amt{
    ...
    private:
        template<typename U>
        using rebind_alloc = typename std::allocator_traits<allocator_type>::template rebind_alloc<U>;
    public:

        template<typename U>
        using rebind = impl::rebind_storage_t<storage_type,U,rebind_alloc<U>>;
    };
    ...
    using dynamic_storage = storage_traits<std::vector<typename A::value_type,A>>;
