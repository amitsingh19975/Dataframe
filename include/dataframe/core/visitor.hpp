...
namespace amt {
    namespace impl {
        ...
        requires( sizeof...( Ts ) > 0ul ) constexpr auto visitor_impl(
            ...
            static_assert( ( traits::InVisitorList<
                                 Ts, typename std::decay_t< S >::type_list > &&
                             ... ),
        ...
        template< traits::BoundedTypeStorage S, typename FnType >
        constexpr auto visitor_impl( S&& storage, FnType&& fn,
    ...
    requires( sizeof...( Ts ) >
        ...
        -> void {
        impl::visitor_impl( std::forward< S >( storage ),
                            std::forward< FnType >( fn ),
                            visitor_list< Ts... > {} );
    }

    template< traits::BoundedTypeStorage S, typename FnType,
              typename TypeList = tag::dummy >
    constexpr auto visit( S&& storage, FnType&& fn, TypeList = {} ) noexcept
        -> void {
        impl::visitor_impl( std::forward< S >( storage ),
                            std::forward< FnType >( fn ), TypeList {} );
    }
