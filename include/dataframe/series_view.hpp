...
template <Box ValueType> struct basic_series_view<ValueType, tag::col_t> {
    ...
    constexpr basic_series_view<ValueType const, axis_tag>
    operator()(slice_type sl) noexcept {
        return {m_data, m_name, m_size, m_dtype, m_slice(sl)};
    }
...
template <Series SeriesType> struct basic_series_view<SeriesType, tag::row_t> {
    ...
    using size_type = typename view_of::size_type;
    using reference = std::conditional_t<std::is_const_v<view_of>,
                                         value_type const &, value_type &>;
    using const_reference = typename view_of::const_reference;
    using pointer = std::conditional_t<std::is_const_v<view_of>,
                                       value_type const *, value_type *>;
    using const_pointer = typename view_of::const_pointer;
    ...
    constexpr basic_series_view(series_pointer_t data, size_type row,
                                size_type col, std::string_view name = "")
        : m_data(data), m_name(name), m_row(row), m_size(col) {}

    template <PureSeries OtherSeriesType>
    requires(is_series_same_v<
             view_of,
             OtherSeriesType>) constexpr basic_series_view(OtherSeriesType
                                                               &&other) {
        if (other.size() != size()) {
            throw std::length_error(
    ...
    requires(is_series_same_v<
             view_of,
             OtherSeriesType>) constexpr basic_series_view(OtherSeriesType const
                                                               &other) {
        if (other.size() != size()) {
            throw std::length_error("amt::basic_series_view(OtherSeriesType "
                                    "const&) : size mismatch");
        }
        parallel_copy(other.begin(), other.end(), begin());
    }
    ...
    constexpr iterator end() noexcept { return {m_data + m_size, m_row}; }

    constexpr const_iterator begin() const noexcept { return {m_data, m_row}; }
    ...
    template <PureSeries OtherSeriesType>
    constexpr operator OtherSeriesType() const {
        OtherSeriesType temp(m_size);
        parallel_transform(begin(), end(), temp.begin(),
                           [](auto &&val) { return val; });
    }

    constexpr name_type name() const noexcept { return m_name; }

    constexpr name_type name() noexcept { return m_name; }
