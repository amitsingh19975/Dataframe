...
#include <dataframe/series_view.hpp>
#include <vector>
...
template <Box ValueType> struct basic_series<ValueType> {
    ...
    // basic_series(dtype)
    constexpr basic_series(DType auto dtype)
        : m_dtype(get<value_type>(dtype)) {}
    ...
    push_back(U u) {
        if constexpr (is_box_v<U>) {
            ...
            m_data.emplace_back(std::move(u), m_dtype);
        }
    }

    template <typename T, typename... Args>
    constexpr void emplace_back(Args &&... args) {
        m_data.emplace_back(T(std::forward<Args>(args)...), m_dtype);
    }
