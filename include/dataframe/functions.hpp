#if !defined(AMT_DATAFRAME_FUNCTIONS_HPP)
#define AMT_DATAFRAME_FUNCTIONS_HPP

#include <dataframe/functions/cast.hpp>
#include <dataframe/functions/concat.hpp>
#include <dataframe/functions/count.hpp>
#include <dataframe/functions/describe.hpp>
#include <dataframe/functions/drop.hpp>
#include <dataframe/functions/filter.hpp>
#include <dataframe/functions/frequency.hpp>
#include <dataframe/functions/head.hpp>
#include <dataframe/functions/info.hpp>
#include <dataframe/functions/maths.hpp>
#include <dataframe/functions/pipeline.hpp>
#include <dataframe/functions/pretty_string.hpp>
#include <dataframe/functions/reduce.hpp>
#include <dataframe/functions/shuffle.hpp>
#include <dataframe/functions/stats.hpp>
#include <dataframe/functions/tail.hpp>
#include <dataframe/functions/transform.hpp>
#include <dataframe/functions/unqiue.hpp>
#include <dataframe/functions/infer.hpp>

#endif // AMT_DATAFRAME_FUNCTIONS_HPP
