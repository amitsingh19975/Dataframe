...
template <> struct concat_t<tag::col_t> {
    ...
    requires(Series<DataFrame> ||
             Frame<DataFrame>) constexpr frame_result_t<FrameTypeIn>
    ...
    requires(Series<DataFrame> || Frame<DataFrame>) constexpr FrameTypeIn &
    operator()(FrameTypeIn &inout, DataFrame const &in, tag::inplace_t) const {
        auto temp = this->operator()(inout, in);
        inout = std::move(temp);
        return inout;
    }
