...
#include <dataframe/functions/stats.hpp>
...
struct describe_t {
  ...
  private:
    ...
    constexpr void numeric_metrics_init(PureFrame auto &out,
    ...
    constexpr void categorical_metrics_init(PureFrame auto &out,
                                            std::size_t cols) const {
        out.resize(cols, 5ul);
        out[0].name("");
        out[0][0] = "count";
    ...
    constexpr void metrics_init(PureFrame auto &out, Series auto const &in,
                                std::size_t cols) const {
        if (is_floating_point(in)) {
    ...
    constexpr void all_metrics_init(PureFrame auto &out,
                                    std::size_t cols) const {
        out.resize(cols, 9ul);
        out[0].name("");
        out[0][0] = "mean";
    ...
    template <typename T> constexpr void all(T const &in, auto &out) const {
