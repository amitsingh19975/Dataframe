...
template <typename T = double> struct var_t {
    constexpr T operator()(Series auto &&s) const {
        T sum{};
        T sq_sum =
            reduce<tag::col_t>(s, T{}, [&sum](T const &acc, T const &val) {
                sum += val;
                return acc + (val * val);
            });
...
template <typename T = double> struct skewness_t {
    constexpr T operator()(Series auto &&s) const {
        ...
        T cub_sum =
            reduce<tag::col_t>(s, T{}, [mn](T const &acc, T const &val) {
                auto temp = val - mn;
...
template <typename T = double> struct covar_t {
    ...
    constexpr T operator()(Series auto &&x, Series auto &&y,
                           tag::sample_t) const {
        auto xmn = mean_t<T>{}(x);
        auto ymn = mean_t<T>{}(y);
        auto c = (x - xmn) * (y - ymn);
...
template <typename T = double> struct minmax_norm_t {
    ...
    constexpr SeriesOut &operator()(SeriesIn const &in, SeriesOut &out) const {
        ...
        auto diff = max - min;
        transform(in, out,
                  [diff, min = min](auto &&val) { return (val - min) / diff; });
        return out;
    }
