...
struct abs_t {
    ...
    requires(Series<DataFrameType> ||
             Frame<DataFrameType>) constexpr DataFrameType
    operator()(DataFrameType const &in) const {
        return transform(
    ...
    requires(Series<DataFrameType> ||
             Frame<DataFrameType>) constexpr DataFrameType &
    operator()(DataFrameType &in, tag::inplace_t) const {
        ...
        if constexpr (!is_frame_v<T>) {
            ...
            if (x.shape() != y.shape()) {
                throw std::runtime_error(ERR_CSTR(
                    "amt::fn::isgreater_t::operator()(T const&, U const&) : "
                    "shape mismatch"));
            }
            return transform(x, y, [this](auto const &xval, auto const &yval) {
                return this->operator()(xval, yval);
...
struct isless_t {
    ...
    operator()(T const &x, U const &y) const {
        if constexpr (!is_frame_v<T>) {
            ...
            if (x.shape() != y.shape()) {
                throw std::runtime_error(ERR_CSTR(
                    "amt::fn::isless_t::operator()(T const&, U const&) : "
                    "shape mismatch"));
            }
            return transform(x, y, [this](auto const &xval, auto const &yval) {
                return this->operator()(xval, yval);
...
struct islessequal_t {
    ...
    operator()(T const &x, U const &y) const {
        if constexpr (!is_frame_v<T>) {
            ...
            if (x.shape() != y.shape()) {
                throw std::runtime_error(ERR_CSTR(
                    "amt::fn::islessequal_t::operator()(T const&, U const&) : "
                    "shape mismatch"));
            }
            return transform(x, y, [this](auto const &xval, auto const &yval) {
                return this->operator()(xval, yval);
