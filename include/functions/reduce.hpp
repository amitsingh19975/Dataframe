...
namespace fn{
    ...
    struct reduce_t<tag::col_t>{
        ...
        constexpr decltype(auto) operator()(Frame auto const &in, Accu&& acc, Fn &&fn) const{

        #pragma omp parallel for schedule(static, 1)
            for (auto i = 0u; i < in.rows(); ++i) {
                acc = std::invoke(std::forward<Fn>(fn), std::forward<Accu>(acc), in.get_row(i));
            }
    ...
    struct reduce_t<tag::row_t>{
        ...
        template <typename Fn>
        constexpr auto& operator()(Series auto const &in, Series auto& out, Fn &&) const{
            out = in;
            return out;
        }
        
        template <typename Fn>
        constexpr decltype(auto) operator()(Frame auto const &in, Series auto&& acc, Fn &&fn) const{

            for (auto i = 0u; i < in.cols(); ++i) {
                acc = std::invoke(std::forward<Fn>(fn), std::forward<decltype(acc)>(acc), in[i]);
            }
...
template<typename T>
inline static constexpr auto reduce = fn::reduce_t<T>{};
