...
#include <core/pipeline_utils.hpp>
...
template <typename... Fns> struct pipeline {
    ...
    requires is_tuple_v<TupleType>
    constexpr decltype(auto) operator()(TupleType&& args,
                                        tag::right_to_left_t) const {
        return invoke_pipeline(m_fns, std::forward<TupleType>(args), tag::right_to_left_t{});
    }

    template <typename TupleType>
    requires is_tuple_v<TupleType>
    constexpr decltype(auto) operator()(TupleType&& args) const {
...
template <Pipeline... Ps> struct connect_pipelines {
    ...
    constexpr connect_pipelines(connect_pipelines const &other) noexcept = default;
    ...
    constexpr decltype(auto) operator()(TupleType&& args,
                                        tag::right_to_left_t) const {
        return invoke_pipeline(m_pipes, std::forward<TupleType>(args), tag::right_to_left_t{});
    }
