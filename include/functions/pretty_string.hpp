...
namespace fn{
    ...
    struct pretty_string_t{
        ...
        requires(Frame<DataFrame> || Series<DataFrame>)
        std::string operator()(DataFrame const& in, std::size_t indent = 4u, bool should_row_number_be_shown = true, std::size_t max_width = 30u){
            std::stringstream ss;
        ...
        std::stringstream& sstream(std::stringstream& ss, FrameType const& in, std::size_t indent = 4u, std::size_t max_width = 30u){
            auto rows = in.rows();
            auto cols = in.cols();
            ...
            for(auto i = 0u; i < cols; ++i) maxes[i] = in[i].name().size();
            ...
            for(auto j = 0u; j < rows; ++j){
                ...
                for(auto i = 0u; i < cols; ++i){
                    ...
                    if(m_row){
                        m_ss << sp << std::string(maxes[i] - str.size(), ' ') << str;
                    }else{
                        m_ss << std::string(maxes[i] - str.size(), ' ') << str << sp;
                    }
                }
            }
            m_ss.swap(ss);
        ...
        std::stringstream& sstream(std::stringstream& ss, SeriesType const& in, std::size_t indent = 4u, std::size_t max_width = 30u){
    ...
    private:
        ...
        constexpr std::size_t count_digit(T val) const noexcept{
            if(val == T{}) return 1u;
            std::size_t ct{};
            while(val){
                ++ct;
        ...
        void process_header(Frame auto const& in, std::vector<std::size_t> const& maxes, std::string const& sp, std::size_t sz){
            auto cols = in.cols();
