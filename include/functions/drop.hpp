...
struct drop_row_t {
    ...
    template <PureSeries SeriesType>
    SeriesType &operator()(SeriesType &in, tag::inplace_t, index_list list) const {
    ...
    template <PureFrame FrameTypeIn>
    FrameTypeIn &operator()(FrameTypeIn &in, tag::inplace_t, index_list list) const {
        auto temp = this->operator()(in, std::move(list));
        in = std::move(temp);
        return in;
    }
...
struct drop_col_t {
    ...
    requires(is_name_list_v<List> || is_index_list_v<List>)
    frame_result_t<FrameTypeIn> operator()(FrameTypeIn const &in,
                                           List list) const {
        frame_result_t<FrameTypeIn> temp;
        this->operator()(in, temp, std::move(list));
    ...
    requires(is_name_list_v<List> || is_index_list_v<List>)
    FrameTypeIn &operator()(FrameTypeIn &in, tag::inplace_t, List list) const {
        auto temp = this->operator()(in, std::move(list));
        in = std::move(temp);
        return in;
    }
