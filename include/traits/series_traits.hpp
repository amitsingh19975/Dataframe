...
template<typename T>
inline static constexpr bool is_pure_series_v = is_series_v< T> && !is_series_view_v<T>;

template<typename T>
concept PureSeries = is_pure_series_v<T>;

template<typename T>
struct series_type;

template<PureSeries T>
struct series_type<T>{
    using type = T;
};

template<SeriesView T>
struct series_type<T>{
    using type = typename T::view_of;
};

template<typename T>
using series_type_t = typename series_type<T>::type;
...
template <typename... Ts> using series_result_t = typename series_result< std::decay_t<Ts>... >::type;
